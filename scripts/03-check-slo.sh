#!/bin/bash
# =============================================================================
# Шаг 3. Проверка SLI/SLO — smoke-тест и метрики
# =============================================================================
...
# Получаем под для каждого сервиса
...
# ---------------------------------------------------------------------------
# 3.2 Smoke-тест: генерируем трафик для метрик
# ---------------------------------------------------------------------------
echo -e "${CYAN}▶ 3.2 Smoke-тест: генерация трафика${NC}"
echo ""

CATALOG_POD=$(kubectl get pod -n ${NS} -l app.kubernetes.io/name=catalog-api -o jsonpath='{.items[0].metadata.name}' 2>/dev/null)
ORDER_POD=$(kubectl get pod -n ${NS} -l app.kubernetes.io/name=order-api -o jsonpath='{.items[0].metadata.name}' 2>/dev/null)

if [ -n "$CATALOG_POD" ]; then
    echo "  Отправляем 20 запросов к catalog-api..."
    SUCCESS=0
    FAIL=0
    TOTAL_TIME=0
    for i in $(seq 1 20); do
        RESULT=$(kubectl exec -n ${NS} ${CATALOG_POD} -- curl -s -o /dev/null -w "%{http_code} %{time_total}" http://localhost:8080/books 2>/dev/null || echo "000 0")
...
if [ -n "$ORDER_POD" ]; then
    echo "  Отправляем 20 запросов к order-api..."
    SUCCESS=0
    for i in $(seq 1 20); do
        CODE=$(kubectl exec -n ${NS} ${ORDER_POD} -- curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/orders 2>/dev/null || echo "000")
        if [ "$CODE" = "200" ]; then
            SUCCESS=$((SUCCESS + 1))
        fi
    done
